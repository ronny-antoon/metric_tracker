...
esp_err_t metric_tracker_init(const char *url_path, const char *device_id, uint16_t max_buffer_size) {
  ...
  ESP_LOGI(TAG, "Initializing metrics server with URL: %s, Device ID: %s, Max Buffer Size: %d", url_path,
           device_id, max_buffer_size);
...
esp_err_t metric_tracker_add_heap() {
  ...
  if (strlen(config_post->json_buffer) == 0) {
    ...
    sprintf(config_post->json_buffer + strlen(config_post->json_buffer),
            ", \"heap\": [{\"name\": \"heap_free\", \"value\": %d}, {\"name\": \"heap_min_free\", \"value\": "
            "%d}]",
...
esp_err_t metric_tracker_add_tasks() {
  ...
  if (strlen(config_post->json_buffer) == 0) {
    // If JSON buffer is empty, initialize it
    sprintf(config_post->json_buffer,
            "{\"device_id\": \"%s\", \"tasks\": [{\"name\": \"%s\", \"stack_free\": %d}",
  ...
  for (x = 1; x < uxArraySize - 1; x++) {
    ...
    sprintf(config_post->json_buffer + strlen(config_post->json_buffer),
            ", {\"name\": \"%s\", \"stack_free\": %d}", pxTaskStatusArray[x].pcTaskName,
            (int)pxTaskStatusArray[x].usStackHighWaterMark);
  }

  // Add the last task
  sprintf(config_post->json_buffer + strlen(config_post->json_buffer),
          ", {\"name\": \"%s\", \"stack_free\": %d}]", pxTaskStatusArray[uxArraySize - 1].pcTaskName,
...
esp_err_t metric_tracker_send(bool send_heap, bool send_tasks) {
  ...
  esp_http_client_config_t config = {
      .url = config_post->url_path, .method = HTTP_METHOD_POST, .timeout_ms = 10 * 1000};
  ...
  // Set POST field
  esp_err_t err =
      esp_http_client_set_post_field(client, config_post->json_buffer, strlen(config_post->json_buffer));
...
esp_err_t metric_tracker_auto_send(uint16_t interval_seconds, bool send_heap, bool send_tasks) {
  ...
  ESP_LOGI(TAG, "Starting auto-send with interval: %d seconds, send_heap: %d, send_tasks: %d",
           interval_seconds, send_heap, send_tasks);
  ON_NULL_PRINT_RETURN(config_post, ESP_ERR_INVALID_STATE, "Config post is NULL, invalid state");
